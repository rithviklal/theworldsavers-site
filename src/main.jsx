...
import {
  ArrowRight, BookOpen, Globe2, HeartHandshake, Lightbulb, Mail,
  MapPin, Rocket, ShieldCheck, Stethoscope, Users
} from 'lucide-react';
  ...
  return (
    <header className="siteHeader">
      ...
      <nav>
        ...
        <a className="navButton" href="https://openvol.theworldsavers.org">Openvol</a>
      </nav>
    </header>
  );
}
  ...
  return (
    <section id="home" className="hero">
      <div className="heroContent">
        <span className="eyebrow"><Globe2 size={16} /> Student-led service, research, and innovation</span>
        ...
        <div className="heroActions">
          <a className="primaryButton" href="#programs">Explore Programs <ArrowRight size={18} /></a>
          <a className="secondaryButton" href="#contact">Join Our Mission</a>
        </div>
        <div className="quickStats">
          <div><strong>{stats.clinics}</strong><span>Clinics Listed</span></div>
          <div><strong>{stats.opportunities}</strong><span>Research & Shadowing</span></div>
      ...
      <div className="heroCard">
        ...
        <div className="impactCard large"><HeartHandshake /><h3>Service</h3><p>Helping students find ways to serve their communities.</p></div>
        <div className="impactCard small"><BookOpen /><h3>Research</h3><p>Connecting curiosity with real opportunities.</p></div>
        <div className="impactCard small"><Rocket /><h3>Innovation</h3><p>Building tools that solve practical problems.</p></div>
      </div>
    </section>
  );
}
  ...
  const programs = [
    {
      ...
      title: 'Openvol',
      status: 'Live',
      description: 'A healthcare volunteering, clinic, shadowing, and research directory for students across Greater Atlanta.',
    ...
    },
    {
      ...
      title: 'Research Hub',
      status: 'Coming Soon',
      description: 'A future resource for student research opportunities, publication guidance, and summer programs.',
      link: '#contact',
      cta: 'Get Updates',
    },
    {
      ...
      status: 'Coming Soon',
      description: 'A future program recognizing community projects, nonprofit leadership, and student service initiatives.',
      link: '#contact',
      cta: 'Learn More',
    },
  ];
      ...
      <div className="programGrid">
        {programs.map((program) => (
          <article className="programCard" key={program.title}>
            ...
            <a href={program.link}>{program.cta} <ArrowRight size={16} /></a>
          </article>
        ))}
      </div>
    </section>
  ...
  return (
    <section id="impact" className="impactSection">
      ...
      <div className="impactGrid">
        <div><strong>{stats.clinics}</strong><span>Healthcare clinics and volunteer sites tracked through Openvol</span></div>
        <div><strong>{stats.opportunities}</strong><span>Shadowing, research, and healthcare exploration opportunities</span></div>
        <div><strong>{stats.counties}</strong><span>Counties represented across Greater Atlanta and Georgia</span></div>
        <div><strong>1</strong><span>Live flagship student impact project and more programs planned</span></div>
      </div>
    </section>
  );
}
    ...
    <section id="founder" className="section founder">
      <div className="founderCard">
        ...
        <div>
          ...
          <h2>Rithvik Lal</h2>
          <p className="founderSub">Lassiter High School • Student Researcher • Future Physician • Community Advocate</p>
          <p>
...
function ContactForm() {
  ...
  async function submitForm(event) {
    ...
    const { error } = await supabase.from('world_savers_contacts').insert({
      name: form.name,
      email: form.email,
      role: form.role,
  ...
  return (
    <section id="contact" className="section contact">
      <div className="contactInfo">
        ...
        <div className="contactLine"><Mail size={18} /><span>contact@theworldsavers.org</span></div>
        <div className="contactLine"><MapPin size={18} /><span>Marietta, Georgia</span></div>
        <div className="privacyLine"><ShieldCheck size={18} /><span>Messages are stored securely in the same Supabase project used by Openvol.</span></div>
      </div>
      <form className="contactForm" onSubmit={submitForm}>
        ...
        <textarea placeholder="Message" value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} />
        <button className="primaryButton" type="submit">Send Message <ArrowRight size={18} /></button>
...
function App() {
  const [stats, setStats] = useState({ clinics: '30+', opportunities: '20+', counties: '10+' });
